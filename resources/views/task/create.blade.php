...
<div class="grid col-span-full">
    ...
    <div class="flex flex-col">
        ...
        <div>
            {{ html()->select('status_id', $taskStatuses->pluck('name', 'id'))->placeholder(null)->class('rounded border-gray-300 w-1/3') }}
        </div>
        @error('status_id')
            <div class="text-rose-600">{{ $message }}</div>
        @enderror
        ...
        <div>
            {{  html()->multiselect('labels[]', $labels->pluck('name', 'id'))->class('rounded border-gray-300 w-1/3 h-32') }}
        </div>
        <div class="mt-2">
            {{  html()->submit(__('views.task.create.buttons.create'))->class('bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded') }}
        </div>
    </div>
    {{ html()->closeModelForm() }}
</div>
